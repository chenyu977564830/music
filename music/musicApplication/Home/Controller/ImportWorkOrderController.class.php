...
class ImportWorkOrderController extends Controller{
	...
	public function excelAdd(){
		...
		$arrCr_id=$basework_order->where("cp_id=$cp_id")->field(array('cr_id','id'))->select();//存放该公司所有的cr_id
		//P($arrCr_id);die;
		...
		//P($arr4);P($arr6);die;
		//var_dump(empty($arr4));die;
		//事物处理

		$basework_order->startTrans();
		if(!empty($arr4)&&!empty($arr6)){//excel表中同时含更新和插入数据处理
			//echo 1;die;
			if($basework_order->addAll($arr4)){
				foreach($arr6 as $v5){
					...
					unset($_SESSION['excelArr']);//清除session
					$status=3;
					$this->ajaxReturn($status);
			}else{
				$basework_order->rollBack();
		...
		}elseif(!empty($arr6)){//excel只含更新字段的处理
			foreach($arr6 as $v5){
					//P($v5);die;
					$result=$basework_order->save($v5);
					if($result===false){
